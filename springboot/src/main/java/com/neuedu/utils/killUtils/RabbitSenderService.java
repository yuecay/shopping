...
public class RabbitSenderService {
    ...
    public void sendKillSuccessEmailMsg(String orderNo){
        System.out.println("秒杀成功异步发送邮件通知消息-准备发送消息:{}"+orderNo);


        try {

            if (StringUtils.isNotBlank(orderNo)){
                KillSuccessUserInfo info=productKillOrderMapper.selectByCode(orderNo);
                if (info!=null){
            ...
            System.out.println("秒杀成功异步发送邮件通知消息-发生异常，消息为");
            e.printStackTrace();
        }
    ...
    /**
     * 秒杀成功后生成抢购订单-发送信息入死信队列，等待一定时间失效超时未支付的订单
     * @param orderNo
     */
    public void sendKillSuccessOrderExpireMsg(final String orderNo) {
        ...
        try {
            ...
            System.out.println("秒杀成功后生成抢购订单-发送信息入死信队列，等待一定时间失效超时未支付的订单-发生异常，消息为");
            e.printStackTrace();
        }
    }
}
